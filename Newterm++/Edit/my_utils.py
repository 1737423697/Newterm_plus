...
def get_response_open_source(prompt, model_name, model, tokenizer):
    ...
    if 'vicuna' in model_name:
        ...
        return qwen2_response(prompt, model, tokenizer)
...
def llama_2_response(prompt, model, tokenizer):
    ...
    question = tokenizer(question, return_tensors="pt")["input_ids"].to(model.device)
    generation_tokens = model.generate(question, num_beams=1, do_sample=False, max_new_tokens=128)[0, question.shape[-1]: ]
    return tokenizer.decode(generation_tokens).rstrip('</s>')
...
def qwen2_response(prompt, model, tokenizer):
    role = ['user', 'assistant']
    messages = [{"role": "system", "content": prompt[0]}]
    for cnt, i in enumerate(prompt[1: ]):    
        messages.append({"role": role[cnt % 2], "content": i})
...
class MultiChat:
    ...
    def __init__(self, config, save_path, retry_func=None, **kwargs):
        if 'claude' in kwargs["model"]:
            ...
            self.api_base = config["openai_base"]
    ...
    def _chat_openai(self, prompt, api_key, api_base,cnt_key,retry=3):
        for cnt in range(retry):
            try:
                ...
                if "encountered an issue with repetitive patterns in your prompt" in str(e):
                    logger.warning('Encountered an issue with repetitive patterns in your prompt\n')
                    return "repetitive patterns", cnt_key  
    ...
    def _chat(self, api_key,api_base):
        ...
        while True:
            try:
                ...
                if 'claude' in self.kwargs["model"]:
                    ...
                    response, cnt_key = self._chat_openai(prompt['prompt'], api_key, api_base,cnt_key)
                if response is None:
                    cnt_key = random.randint(0, len(api_key) - 1)
    ...
    def start(self):
        ...
        for keys in self.api_keys:
            p = Process(target=self._chat, args=[keys, self.api_base])
            p.start()
...
def eval_qa(model, tokenizer, meanings, task, model_name, path, year=2023):
    pattern = re.compile('[\W_]+')

    task_file = os.path.join(path, f'{task}_rag.jsonl')
    ...
    with open(task_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Check if the term is in the list
            if meaning in meanings:
                for p in range(3):
                    ...
                    # Get model response
                    line['response'] = get_response_open_source(prompt, model_name.capitalize(), model, tokenizer)
                    ...
                    if task == 'CSJ':
                        ...
                        # Clean and extract options from the response
                        ...
                        if selected_option == ['A', 'B', 'C', 'D'][line['gold']]:
                            cor += 1
                        elif selected_option not in ['A', 'B', 'C', 'D']:
                        ...
                        if 'A' not in res and 'B' not in res and 'C' not in res and 'D' not in res:
                            cnt = sum(1 for choice in line['choices'] if choice.lower() in line['response'].lower())
                            if cnt != 1:
                                inv += 1  # Count invalid responses

                    # Add each result entry to the results list
                    results.append({
                        'prompt': prompt,
